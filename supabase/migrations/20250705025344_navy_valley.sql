...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_match_preferences_updated_at') THEN
...
-- 在用户活动时自动更新最后活跃时间（只在不存在时创建）
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_last_active_trigger') THEN
