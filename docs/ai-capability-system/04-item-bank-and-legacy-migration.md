# PACF 候选题库、随机出题与旧数据迁移说明 v1.1

> 日期：2026-08-09  
> 状态：旧版迁移已在线执行并通过完整性校验；题库随机出题迁移待执行
...
- v1.0 为固定30道，六维各5道；v1.1 为每次随机42道，六维各7道；
- 每个维度先保证5个核心能力单元各抽1道客观/情境题，再从该维度剩余可用题中补抽2道；
- 预计 15–18 分钟；
...
- 作者输入按“弱→强”便于审稿，题库生成时已按能力 ID 轮换选项位置；每次会话由服务端随机题目顺序，并按选项 ID 判分。后续若增加选项随机化，必须把本次选项顺序写入会话快照，不能让最高分长期固定在同一位置。
- 浏览器只接收题干和无分值选项；`score`、rubric 内部说明和判分理由不得进入前端包或接口响应。
- 同一用户复测应优先抽平行题，避免记忆答案；当前版本已记录会话快照，可在下一次题库迭代按历史会话避开近期题目；题目曝光量写入运营表。
...
| `ai_assessment_sessions` | 本次随机抽到的题目、顺序与评分快照 | 服务端，过期或完成后不可重用 |
| `ai_style_items` | 风格题目、偏好方向和行为题配置 | 服务端，严禁返回方向键 |
...
3. 冻结预测试题库 `pacf-item-bank-1.1.0`，并在本迁移中一次写入120道候选题。
4. 在 Supabase SQL Editor 执行 `20260810102109_assessment_item_bank_sampling_v1_1.sql`。
...
- 120 道候选题，ID 全部唯一；
- 30 个能力单元均恰好包含四种题型；
- v1.1 随机快测每次42道，六维均为7道，并且每维覆盖5个核心能力单元；
