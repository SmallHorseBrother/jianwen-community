...
- 第一届 AI 能力摸底考试（2026 试行卷）：服务端保存54道混合题型候选题，每次按六维分层抽取30道计分题，再抽2道不计分主观题，共32题；包含基础选择、情境判断、填空计算、代码与流程阅读和开放作答，输出Level 0–5和六维雷达图。
- AI使用风格随机画像：完整题库有56道计分陈述和4道行为题；每次每条风格轴按两侧各抽4道，共32道计分题加4道行为题，输出四条0–100连续轴、平衡提示、四字母编码和16种中文角色名。
- 每次开始测评都会生成一份仅服务端可见的题目与评分快照，并设置有效期；因此题库日后更新不会改变历史结果，也不能用别人的题目清单提交答案。
...
supabase/migrations/20260811203644_first_ai_exam_2026_v1.sql
```
...
3. 连续开始两次能力考试，确认各次均为32题（30道计分题、2道主观题），五部分数量为12/6/6/6/2，题目顺序或抽取内容有所不同；完成后得到Level 0–5和六维雷达图。
